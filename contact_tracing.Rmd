...
It may not seem obvious, but the observations in the `relationships` data represent contact events between sources and targets. To recognize that better, we `count`events in both the source and target variables.
...
The two main ingredients of a network graph are __nodes__ and __edges__. Nodes can be thought of as the units of observation, and edges are the threads that connect nodes together. To make the nodes, we fork our target and source variables into seperate objects that will become distinct units of observation, in this case unique __people__ who are included in the contact tracing investigation. 
...
Once we have a list of the distinct nodes with unique is values, we can join `ct_nodes` to a new fork of the `relationships` data that will make our graph edges. The `id` variable is joined in two different ways: from the `source_visualid` as `from` and the `target_visualid` as `to`.
...
ct_edges <- relationships %>% 
  ...
  rename(from = id) %>% 
   # then join the targets and call rename those ids as to
...
The `ct_edges` object now contains all the observable connections between distinct nodes, directionalilty is obtained through the `to` and `from` variables. `ct_nodes` and `ct_edges` are the bare essentials required for a network graph. With them, we can put togther a fairly nice interactive graph using the `visNetwork` package. We can also add  a few basic options, like a node id selection dropdown.  
...
This interactive object is a visualization of the connections __from__ sources (cases) __to__ targets (contacts) in the `relationships` dataset. I strongly suggeest zooming in and out of the graph in your browser, clicking and dragging on the nodes helps make sense of the shapes together. Can we learn anything about this outbreak from the network graph?

The visualization is interesting to look at and play around with. In many ways it is only a beginning, an exploratory view into what network analysis can offer the field of contact tracing. 
